  ...
  try {
    ...
    for (const t of due) {
      ...
      try {
        ...
        await tx.commit();
        console.log(
          `[recurring] "${t.description || t.merchant}" → nächste Buchung: ${nextDate.toISOString().split("T")[0]}`
        );
      } catch (err) {
        await tx.rollback();
        console.error(
          `[recurring] Fehler bei "${t.description || t.merchant}": ${err.message}`
        );
      }
    }
  } catch (err) {
...
async function syncAllPaperless() {
  try {
    ...
    const configs = await PaperlessConfig.findAll({
      where: { isActive: true },
    });
    if (configs.length === 0) {
      return;
    ...
    console.log(
      `[paperless-sync] Synchronisiere ${configs.length} Haushalt(e)...`
    );
    ...
    for (const config of configs) {
      try {
        ...
        const fetchAll = async (url) => {
          const results = [];
          let nextUrl = url;
          const _origin = new URL(baseURL).origin;
          while (nextUrl) {
        ...
        let users = [];
        try {
          users = await fetchAll(`${baseURL}/api/users/`);
        } catch {}
        ...
        const bulkUpsert = async (table, rows, conflictCols, updateCols) => {
          ...
          await sequelize.query(
            `INSERT INTO ${table} (${cols.map((c) => `"${c}"`).join(",")}) VALUES ${ph} ON CONFLICT (${conflict}) DO UPDATE SET ${updates}`,
            { bind: vals }
          );
        };
        if (docTypes.length) {
        ...
        console.log(
          `[paperless-sync] Haushalt ${hid}: ${docTypes.length} Typen, ${correspondents.length} Korrespondenten, ${tags.length} Tags, ${users.length} Benutzer`
        );
      } catch (err) {
        console.error(
          `[paperless-sync] Haushalt ${config.householdId}: ${err.message}`
        );
      }
    }
  } catch (err) {
...
async function startCron() {
  ...
  try {
    ...
    if (config?.isActive && config?.schedule) {
      ...
      console.log(
        `[cron] Backup scheduled: ${config.scheduleLabel || config.schedule}`
      );
    }
  } catch (e) {
...
function stopCron() {
  if (recurringJob) {
    recurringJob.destroy();
    recurringJob = null;
  }
  if (paperlessJob) {
    paperlessJob.destroy();
    paperlessJob = null;
  }
  if (reportsJob) {
    reportsJob.destroy();
      ...
      try {
        ...
        for (const m of members) {
          await mailer.sendMail({
            ...
            to: m.User.email,
            subject: `Haushaltsbuch Monatsbericht ${monthNames[lastMonth - 1]} ${year}`,
            html,
          });
        }
        console.log(
          `[reports] Monatsbericht ${h.name}: ${members.length} E-Mails gesendet`
        );
      } catch (err) {

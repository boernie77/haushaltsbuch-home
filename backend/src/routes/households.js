...
router.post("/", auth, async (req, res) => {
  try {
    ...
    const household = await Household.create({
      ...
      monthlyBudget,
      budgetWarningAt: budgetWarningAt || 80,
      isShared,
...
router.put("/:id", auth, async (req, res) => {
  try {
    ...
    const member = await HouseholdMember.findOne({
      where: { householdId: req.params.id, userId: req.user.id },
    });
  ...
  try {
    ...
    const members = await HouseholdMember.findAll({
      ...
      include: [
        {
          model: User,
          attributes: ["id", "name", "email", "avatar", "lastLoginAt"],
        },
      ],
    });
    res.json({ members });
...
router.post("/:id/invite", auth, async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ invite, inviteLink: `${process.env.APP_URL}/join/${code}` });
  } catch (err) {
    ...
    if (updates.anthropicApiKey && aiEnabled) {
      try {
        ...
        return res.status(400).json({
          ...
          message:
            "Der API-Key ist ungültig. Bitte überprüfe ihn auf console.anthropic.com.",
        });
      }
    }
...
router.delete("/:id", auth, async (req, res) => {
  try {
    ...
    const membership = await HouseholdMember.findOne({
      where: { householdId: id, userId: req.user.id, role: "admin" },
    });
    if (!membership) {
      return res
        .status(403)
        .json({ error: "Nur der Haushalt-Admin kann löschen" });
    }

    // Sicherstellen, dass der Nutzer noch mindestens einen anderen Haushalt hat
    const otherHouseholds = await HouseholdMember.count({
      where: {
        userId: req.user.id,
        householdId: { [require("sequelize").Op.ne]: id },
      },
    });
    if (otherHouseholds === 0) {
      return res
        .status(400)
        .json({ error: "Das letzte Haushaltsbuch kann nicht gelöscht werden" });
    }
...
router.delete("/:id/members/:userId", auth, async (req, res) => {
  try {
    const admin = await HouseholdMember.findOne({
      where: { householdId: req.params.id, userId: req.user.id },
    });
    ...
    await HouseholdMember.destroy({
      where: { householdId: req.params.id, userId: req.params.userId },
    });

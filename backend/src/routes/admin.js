...
router.get("/stats", auth, superAdminGuard, async (req, res) => {
  try {
    ...
    const adminUserIds = await Household.findAll({
      attributes: [
        [sequelize.fn("DISTINCT", sequelize.col("adminUserId")), "adminUserId"],
      ],
      raw: true,
    });
    ...
    res.json({
      userCount,
      householdCount: householdAdminCount,
      transactionCount,
    });
  } catch (err) {
...
router.get("/users", auth, superAdminGuard, async (req, res) => {
  try {
    ...
    adminHouseholds.forEach((h) => {
      if (!grouped[h.adminUserId]) {
        grouped[h.adminUserId] = [];
      }
    ...
    const usersWithHouseholds = users.map((u) => ({
      ...
      inviteUsedAt: null, // populated via InviteCode below
    }));

    // Attach invite activation dates
    const usedInvites = await InviteCode.findAll({
      where: { usedById: adminUserIds },
      attributes: ["usedById", "usedAt"],
    });
    const inviteMap = {};
    usedInvites.forEach((inv) => {
      if (!inviteMap[inv.usedById]) {
        inviteMap[inv.usedById] = inv.usedAt;
      }
    });
    usersWithHouseholds.forEach((u) => {
      u.inviteUsedAt = inviteMap[u.id] || null;
    });
...
router.put("/ai-settings", auth, superAdminGuard, async (req, res) => {
  try {
    ...
    if (!settings) {
      settings = await GlobalSettings.create({ id: "global" });
    }
    ...
    if (apiKey && apiKey.trim()) {
      ...
      try {
        ...
        return res.status(400).json({
          ...
          message: "Der API-Key ist ungültig.",
        });
      }
...
router.post("/invite-codes", auth, superAdminGuard, async (req, res) => {
  try {
    ...
    const expiresAt = expiresIn
      ? new Date(Date.now() + Number.parseInt(expiresIn) * 3600 * 1000)
      : null;
...
router.get("/backup/ssh-key", auth, superAdminGuard, async (req, res) => {
  try {
    ...
    if (!global) {
      global = await GlobalSettings.create({ id: "global" });
    }
...
router.get("/backup/config", auth, superAdminGuard, async (req, res) => {
  try {
    ...
    res.json({
      hasConfig: true,
      config: { ...safe, hasPassword: !!sftpPassword },
    });
  } catch (err) {
...
router.put("/backup/config", auth, superAdminGuard, async (req, res) => {
  try {
    ...
    const updates = {
      ...
      sftpUser,
      sftpPath: sftpPath || "/backups",
      schedule,
      scheduleLabel,
      isActive: !!isActive,
    };
    if (sftpPassword && sftpPassword.trim()) {
      updates.sftpPassword = sftpPassword.trim();
    }
    ...
    if (config) {
      ...
      config = await BackupConfig.create(updates);
    }

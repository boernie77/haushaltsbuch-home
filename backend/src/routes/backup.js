...
const { exportHouseholdData, importHouseholdData } = require('../services/backupService');
...
router.get('/export', auth, async (req, res) => {
  ...
  const member = await HouseholdMember.findOne({ where: { householdId, userId: req.user.id } });
  ...
  try {
    ...
    if (format === 'csv') {
      ...
      res.setHeader('Content-Disposition', `attachment; filename="haushalt-export-${date}.csv"`);
      return res.send('\uFEFF' + data); // UTF-8 BOM for Excel
    }

    res.setHeader('Content-Type', 'application/json');
    res.setHeader('Content-Disposition', `attachment; filename="haushalt-export-${date}.json"`);

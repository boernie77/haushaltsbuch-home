...
async function buildMonthlyReport(householdId, year, month) {
  ...
  const categoryRows = byCategory
    ...
    <tr>
      ...
      <td style="text-align:right">${fmt(Number.parseFloat(b.dataValues.total))}</td>
    </tr>`
    )
    .join("");
  ...
  const txRows = transactions
    ...
    <tr>
      ...
      <td style="text-align:right;color:${t.type === "income" ? "#16a34a" : "#dc2626"}">${t.type === "income" ? "+" : "−"}${fmt(Number.parseFloat(t.amount))}</td>
      <td>${t.User?.name || ""}</td>
    </tr>`
    )
    .join("");
...
router.get("/monthly", auth, async (req, res) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="bericht-${y}-${String(m).padStart(2, "0")}.html"`
    );
    res.send(html);
  } catch (err) {
    console.error(err);
...
router.post("/send-monthly", auth, async (req, res) => {
  try {
    ...
    if (!mailer) {
      return res
        .status(503)
        .json({ error: "E-Mail nicht konfiguriert (SMTP_HOST fehlt)" });
    }

    const m = Number.parseInt(month) || new Date().getMonth();
    const y = Number.parseInt(year) || new Date().getFullYear();
    const data = await buildMonthlyReport(householdId, y, m);
    const html = renderHtml(data);

...
function encrypt(plaintext) {
  ...
  const key = getKey();
  if (!key) return plaintext; // No key configured → store plaintext (dev mode)
...
function decrypt(ciphertext) {
  ...
  if (parts.length !== 3) return ciphertext; // Plaintext legacy value
  ...
  try {
    ...
    if (iv.length !== IV_BYTES) return ciphertext;
